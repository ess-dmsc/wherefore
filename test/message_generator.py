from streaming_data_types import serialise_f142, serialise_ev42, serialise_hs00, serialise_ns10, serialise_pl72,\
    serialise_6s4t, serialise_x5f2, serialise_ep00, serialise_tdct, serialise_rf5k, serialise_answ, serialise_wrdn,\
    serialise_ndar, serialise_ADAr
...
def create_hs00_test_data(name, time):
    return {
        "source": name, "timestamp": time, "current_shape": [5],
        "dim_metadata": [
            {
                "length": 5,
                "unit": "m",
...
list_of_serialisers = [
    ...
    lambda name, time: serialise_ns10(key=name, value="42", time_stamp=time/1e9, ttl=111),
    lambda name, time: serialise_pl72("some_job_id", "some_file_name.nxs", int(time/1e6), None, service_id=name),
    lambda name, time: serialise_6s4t("some_job_id", service_id=name, command_id="some_command_id", stop_time=datetime.now()),
    lambda name, time: serialise_x5f2("some_app_name", "v1.2.3", name, "some_host_name", 123456, 65431, "{}"),
    ...
    lambda name, time: serialise_answ(name, "some_job_id", "some_command_id", action=ActionType.StartJob, outcome=ActionOutcome.Failure, message="Some message.", status_code=432, stop_time=datetime.now()),
    lambda name, time: serialise_wrdn(name, "some_job_id", False, "some_file_name.hdf"),
    lambda name, time: serialise_ADAr(name, 1234, datetime.fromtimestamp(time/1e9), np.linspace(0, 100)),
...
def main():
    ...
    while True:
        for i in range(args.n):
            producer.send(topic=args.topic, value=list_of_serialisers[i % len(list_of_serialisers)](used_names[i], int(time.time()*1e9)))
        time.sleep(1)
